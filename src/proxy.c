...
int start_proxy(const proxy_config_t *config) {
  ...
  err = getaddrinfo(config->listen_host, port_str, &hints, &res);
  if (err != 0) {
    log_error("Failed to resolve or bind listening address: %s",
              gai_strerror(err));
    return -1;
  }
  ...
  while (proxy_running) {
    ...
    strncpy(ctx->target_host, config->target_host,
            sizeof(ctx->target_host) - 1);
    ctx->target_host[sizeof(ctx->target_host) - 1] = '\0';
    ctx->target_port = config->target_port;
    pthread_t thread;
    err = pthread_create(&thread, NULL, proxy_thread_func, ctx);
    if (err != 0) {
